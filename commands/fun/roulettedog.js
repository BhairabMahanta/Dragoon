		...
		try {
			...
			if (args.length < 4) {
				return message.reply(
					'Usage: c!roulettedog #channelId <spawnchance> <interval> <emote> <participantLimit> <killLimit> <minimumParticipants> <inactiveLimit> ',
				);
			}
			...
			return message.reply(
				`Successfully set up a roulette game in <#${channelId}> with a spawn chance of ${spawnChance}% every ${interval} minutes! (${emote}). The game will end once ${kLimit} participants have been shot or ${iLimit} minutes pass`,
			);
		} catch (e) {
			console.error(e);
			return message.reply(
