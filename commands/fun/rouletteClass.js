	...
	async start() {
		...
		const embed = new EmbedBuilder()
			...
			.addFields({
				...
				value: `${this.participants.length}/${daHta.participantLimit}`,
				inline: false,
			});

		// Create the action row with the button
		const row = new ActionRowBuilder().addComponents(
			new ButtonBuilder().setCustomId('join_roulette').setLabel('Join Roulette').setStyle('Primary'),
		);

		// Send the initial embed with the button
		this.sentMessage = await this.client.channels.cache
			.get(this.channelId)
		...
		collector.on('collect', async (interaction) => {
			try {
				if (
					!this.participants.includes(interaction.user.id) &&
					!(this.participants.length >= this.daHta.participantLimit)
				) {
				...
				if (this.participants.length >= daHta.participantLimit) {
					this.isDone = true;
					console.log('isdone old?:', this.isDone);
					await startRouletteOrOtherAction(
						this.participantsToSplice,
						this.participants,
						this.daHta,
						this.sentMessage,
		...
		collector.on('end', async () => {
			const elapsedTime = Date.now() - this.startTime;
			console.log('Elapsed time it ended bitch:', elapsedTime);
			console.log('isdone?:', this.isDone);
			if (
				(this.participants.length >= daHta.participantLimit && !this.isDone) ||
				(elapsedTime >= daHta.inactiveLimit &&
					this.participants.length >= daHta.minimumParticipants &&
					this.isDone == false)
			) {
				await startRouletteOrOtherAction(
					this.participantsToSplice,
